...
int main(int argc, char **argv) {
    ...
    // Claves compartidas (rellenar con tus valores reales)
    ...
    if (argc < 2 && rank == 1) {
        fprintf(stderr, "[Cliente] Uso: %s <archivo_a_enviar>\n", argv[0]);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    if (rank == 0) {
        run_server(num_processes, alicesk, bobpk);
    } else if (rank == 1) {
        run_client(argv[1], bobsk, alicepk);
    } else {
        run_node(rank, alicesk, bobpk);
    }
